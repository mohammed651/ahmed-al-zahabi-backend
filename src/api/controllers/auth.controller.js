...
function signRefreshToken() {
  // use uuid for refresh token value (stored hashed in DB in production ideally)
  return uuidv4();
}
...
export async function refresh(req, res) {
  try {
    ...
    console.error(err);
    return respError(res, 'فشل في تجديد التوكن', 500);
  }
}
...
export async function logout(req, res) {
  try {
    ...
    console.error(err);
    return respError(res, 'فشل في تسجيل الخروج', 500);
  }
}
